...
**可视化**
- 6 种渲染模式：`event_frame`（ON=绿/OFF=红，衰减） · `time_surface`（热力图） · `on_only` · `off_only` · `accumulated`（累积不衰减） · `instant`（瞬时帧，无衰减，消除超高速横纹）
- 衰减系数实时调节（0.01–0.99）
- 事件切片 Δt 可调（100µs–100ms）；高速场景推荐 instant 模式 + 200–500µs
...
| 普通相机 | DVS 相机 |
|----------|----------|
| ![普通相机：激光点不可见](demo/激光红外%28本身红外不可见%29.jpg) | ![DVS相机：激光轨迹清晰可见](demo/激光红外红外.png) |
...
**Visualization**
- 6 rendering modes: `event_frame` (ON=green / OFF=red, decay) · `time_surface` (heatmap) · `on_only` · `off_only` · `accumulated` · `instant` (no decay, eliminates streak artifacts at ultra-high speed)
- Real-time decay factor slider (0.01–0.99)
- Adjustable event slice Δt (100µs–100ms); for fast targets use `instant` + 200–500µs
...
![People Motorcycles Cars](demo/人摩托车汽车.mp4)
...
|---------------|------------|
| ![Normal camera: laser invisible](demo/激光红外%28本身红外不可见%29.jpg) | ![DVS camera: laser trail visible](demo/激光红外红外.png) |
...
**可視化**
- 6種類のレンダリングモード：`event_frame` / `time_surface` / `on_only` / `off_only` / `accumulated` / `instant`（瞬時フレーム、超高速横縞アーティファクトを解消）
- リアルタイム減衰係数スライダー
- イベントスライス Δt 調整可能（100µs–100ms）
...
| 通常カメラ | DVS カメラ |
|-----------|-----------|
| ![通常カメラ：レーザー不可視](demo/激光红外%28本身红外不可见%29.jpg) | ![DVSカメラ：レーザー軌跡可視](demo/激光红外红外.png) |
